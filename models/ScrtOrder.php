...
class ScrtOrder extends stdClass {
  ...
  public function store($trigger_integrations = true) {
    ...
    if( !$og_order->id && $this->id || ($og_order->id && $og_order->status != $this->status) ) {
      ...
      $trigger_integrations = apply_filters('sc_trigger_order_integrations', $trigger_integrations, $this);
  ...
  public function load_from_post() {
    ...
    do_action('sc_after_setup_atts_from_post',$this,$_POST);
    ...
    do_action('sc_order_pre_calculate_tax', $this);
  ...
  public function apply_plan_coupon_to_items() {
    ...
    if ($this->coupon['type'] == 'percent' || $this->coupon['type'] == 'fixed') {
        foreach($this->items as $k=>$item) {
            ...
            $amt = ($this->plan->type == 'recurring') ? $this->plan->price : $item['total_amount'];
  ...
  public function add_bump_items_from_post($bumps) {
    ...
    foreach($bumps as $i=>$bump) {
        ...
        $price_name = sc_get_public_product_name($ob_id);
        $type = $price_id = 'bump';
