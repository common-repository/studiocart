...
class NCS_Cart_Admin {
	...
	private function load_dependencies() {
		...
		require_once plugin_dir_path( dirname( __FILE__ ) ) .  'admin/class-ncs-cart-dashboard.php';
		require_once plugin_dir_path( dirname( __FILE__ ) ) .  'admin/class-ncs-cart-search-filters.php';
...
<p>Please see the <a href="https://stripe.com/privacy">Stripe Privacy Policy</a> for more details.</p>',
            ...
            <div class="sc-product-info sc-product-table meta-box-sortables ui-sortable">
                ...
                <table cellpadding="0" cellspacing="0" class="sc_order_items" width="100%">
                    ...
                    <tbody id="order_line_items">
                        ...
                        <?php
                        if(!empty($subID)) { 
                            $sub = new ScrtSubscription($subID);
                            ?>
                        ...
                        do_action('sc_order_related_orders', $order);
	...
	*/
	...
	public function subscription_form_callback( $post ){
        ...
        $order_obj = new ScrtSubscription($post->ID);
        $order = (object) $order_obj->get_data();
		...
		<div class="sc-product-info sc-product-table meta-box-sortables ui-sortable">
			...
			<table cellpadding="0" cellspacing="0" class="sc_order_items" width="100%">
					...
					<tbody id="order_line_items">
                        ...
                        do_action('sc_subscription_related_orders', $order_obj);
            ...
            foreach ( $object_ids as $post_id ) {
                ...
                switch($post_type){
                    ...
                    case "sc_subscription":
                        ...
                        if('bulk_'.$doaction == 'bulk_sc_make_active') {
                            $sc_subscription->status = $status_to;
                            ScrtSubscription::update($sc_subscription);
                        } else {
                            $out = sc_do_cancel_subscription($sc_subscription, $sc_subscription->subscription_id, $now=true, $echo=false);
                            if($out == 'OK') {
                                sc_log_entry($sc_subscription->id, __("Subscription canceled by admin", 'ncs-cart'));
                            } else {
                                sc_log_entry($sc_subscription->id, sprintf(__("Error canceling subscription! Message: %s", 'ncs-cart'), $out));
                            }
                            if('bulk_'.$doaction == 'bulk_sc_make_completed') {
                                $sc_subscription->status = 'completed';
                                $sc_subscription->sub_status = 'completed';
                            }
                        }
                    break;
                    default:
